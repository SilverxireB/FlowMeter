...
export default function SahneManagePage() {
  ...
  async function yukle(files: File[]) {
    ...
    const failed = files.filter((f) => !f.type.startsWith("image/")).map((f) => `${f.name} (yalnız fotoğraf)`);
    let n = 0;
    for (let i = 0; i < ok.length; i++) {
      try {
        setQueue({ done: i, total: ok.length, pct: 0 });
        await uploadSahneFoto(sahne.id, ok[i], (pct) => setQueue({ done: i, total: ok.length, pct }));
        n++;
      } catch (e) {
    ...
    <main className="min-h-screen bg-wash">
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          <button
            onClick={() => (window.history.length > 1 ? router.back() : router.push("/screens"))}
          ...
          <div className="rounded-2xl border border-line bg-white p-4">
            ...
            {queue ? (
              ...
              <button
                ...
                className="w-full mb-3 rounded-xl border-2 border-dashed border-line hover:border-accent hover:bg-accent-soft/20 text-muted hover:text-accent px-3 py-4 flex flex-col items-center gap-1.5 transition-colors group"
              >
              ...
              <ul className="flex flex-col gap-2">
                {(sahne.fotolar ?? []).map((f) => (
                  <li key={f.src} className="flex items-center gap-2.5 rounded-xl border border-line p-2">
                    ...
                    <img src={f.src} alt="" className="w-14 h-14 rounded-lg object-cover bg-black shrink-0" />
                    <input
                      defaultValue={f.yazi ?? ""}
      ...
      {silOnay && (
        <ConfirmDialog
          ...
          onConfirm={() => {
            const f = silOnay;
            setSilOnay(null);
            void updateSahne(sahne.id, { fotolar: (sahne.fotolar ?? []).filter((x) => x.src !== f.src) }).catch(() =>
              setErr("Silinemedi.")
            );
          }}
