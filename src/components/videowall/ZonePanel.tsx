...
/**
 * FlowSign içerik paneli (v3 + UX cila). Seçili alana içerik ata: görsel/video
 * (Cloudinary), URL, METİN (duyuru), SAAT (canlı widget). İçerik alanın
 * çözünürlüğüne STRETCH edilir (kırpma/boşluk yok) — sığdır/doldur seçeneği YOK.
 * Öğeler SÜRÜKLE-BIRAK ile sıralanır; öğe başına süre + saat aralığı.
...
/** Küçük liste önizlemesi (tanıma amaçlı). */
function ItemThumb({ item }: { item: ZoneItem }) {
  ...
  if (item.kind === "video" && item.src) {
    const still = cldFit(item.src, 160).replace(/\.(mp4|mov|webm|m4v)$/i, ".jpg");
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={still} alt="" className={`${base} object-cover bg-black`} />;
  }
  if (item.kind === "text")
    return (
      <div className={`${base} font-bold text-sm`} style={{ background: item.bg ?? "#0c3b3b", color: item.color ?? "#fff" }}>
        Aa
      </div>
    );
  if (item.kind === "clock")
    return (
      <div className={`${base} text-xl`} style={{ background: item.bg ?? "#041a1a", color: item.color ?? "#fff" }}>
        🕐
      </div>
    );
...
export default function ZonePanel({
  ...
  async function onFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        added.push({ id: iid(), kind: res.type, src: res.url, name: file.name.replace(/\.[^.]+$/, ""), durationSec: res.type === "image" ? 8 : undefined });
      } catch (e) {
  ...
  const ADD_BTNS: { label: string; icon: string; fn: () => void }[] = [
    { label: "Görsel / Video", icon: "🖼", fn: () => fileRef.current?.click() },
  ...
  return (
    <div className="rounded-2xl bg-white/[0.06] border border-white/10 p-5">
      ...
      <div className="flex items-center gap-2 mb-5">
        ...
        />
        {cells > 1 && (
          <button onClick={onSplit} className="shrink-0 rounded-lg border border-white/15 text-white/70 hover:border-white/40 px-3 py-1.5 text-xs font-semibold">
            ⛶ Böl
          </button>
        )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
        {ADD_BTNS.map((b) => (
          <button
            key={b.label}
            onClick={b.fn}
            disabled={progress !== null && b.label.startsWith("Görsel")}
            className="rounded-xl bg-white/[0.06] border border-white/10 hover:border-[#2dd4bf]/50 hover:bg-white/10 px-3 py-3 text-sm font-semibold flex flex-col items-center gap-1 transition-colors disabled:opacity-50"
          >
      ...
      {progress !== null && (
        <div className="mb-4">
          <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
            <div className="h-full bg-[#2dd4bf] transition-[width]" style={{ width: `${progress}%` }} />
          </div>
          <p className="text-white/50 text-xs mt-1 tabular-nums">Yükleniyor… {progress}%</p>
        </div>
      )}
      ...
      {zone.items.length === 0 ? (
        <div className="text-center py-10 text-white/35 border border-dashed border-white/10 rounded-xl">
          ...
          <p className="text-sm">Bu alan boş. Yukarıdan içerik ekle.</p>
        </div>
      ) : (
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 pl-8 text-xs text-white/60">
                ...
                {it.kind !== "video" && (
                  <label className="flex items-center gap-1.5">
                    Süre
                    <input type="number" min={2} defaultValue={it.durationSec ?? 8} onBlur={(e) => patchItem(it.id, { durationSec: Math.max(2, Number(e.target.value) || 8) })} className="w-14 rounded bg-white/10 border border-white/15 px-2 py-1 tabular-nums focus:outline-none focus:border-[#2dd4bf]" />
                    sn
                  </label>
                )}
                <label className="flex items-center gap-1.5">
      ...
      <p className="text-white/35 text-[11px] mt-3">İçerik alana tam oturur (stretch). Birden çok öğe sırayla döner · saat aralığı boşsa hep görünür · sürükle-bırak ile sırala.</p>
    </div>
  );
}
