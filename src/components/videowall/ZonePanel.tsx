...
function ItemThumb({ item }: { item: ZoneItem }) {
  ...
  if (item.kind === "fotoSahne")
    return item.fotolar?.length ? (
      // eslint-disable-next-line @next/next/no-img-element
      <img src={cldFit(item.fotolar[0], 160)} alt="" className={`${base} object-cover`} />
    ) : (
...
export default function ZonePanel({
   ...
   * Süre uzun başlar: Wall modlarının ritmi saatlerce açık kalan bir perdeye
   * göre; 30 saniyede mod ısınamadan biter.
   */
  const addFotoSahne = () =>
    setItems([
      ...zone.items,
                        ...
                        {(it.fotolar?.length ?? 0) > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {(it.fotolar ?? []).map((f) => (
                              ...
                              >
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={cldFit(f, 120)} alt="" className="w-full h-full object-cover bg-black" />
