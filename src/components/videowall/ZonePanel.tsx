...
import { createSahne } from "@/lib/sahneler";
import { RafDosyasi, rafListesi, rafaKoy, raftanSil } from "@/lib/ortakRaf";
...
function ItemThumb({ item }: { item: ZoneItem }) {
  ...
  if (item.kind === "video" && item.src) {
    ...
    return <div className={`${base} bg-ink/80 text-xl`}>🎬</div>;
  }
  if (item.kind === "text")
...
export default function ZonePanel({
  ...
  // Liste iki sebeple yüklenir: (1) seçici açıldığında, (2) alanda ZATEN bağlı
  // ekran varsa — çünkü o öğenin adını TAZE göstermek ve hedef silinmişse
  // uyarmak gerekiyor. Eskiden ad, bağlandığı andaki hâliyle donuyordu ve
  // hedef silinince alan sessizce boşalıyor, editörde hiçbir iz kalmıyordu.
  ...
  // Raf sekmesi açılınca listeyi SUNUCU GERÇEĞİNDEN çek (kayıt yok — depo listelenir).
  useEffect(() => {
  ...
  async function uploadFilesRaf(files: File[]) {
    setErr(null);
    if (!cloudReady) {
      setErr("Medya deposu yapılandırılmadı — görsel/video yüklenemez.");
      return;
    }
    const { ok, rejected } = precheck(files);
    const failed: string[] = [...rejected];
    ...
    for (let i = 0; i < ok.length; i++) {
      try {
        setQueue({ done: i, total: ok.length, pct: 0 });
        await uploadToCloudinary(ok[i], "flowsign/ortak", (pct) => setQueue({ done: i, total: ok.length, pct }), { keepOriginal: true });
        yuklendi++;
      } catch (e) {
        failed.push(`${ok[i].name} (${e instanceof Error ? e.message : "yükleme hatası"})`);
      }
  ...
  async function fotoSahneOlustur(ad: string) {
    ...
    try {
      const u = auth().currentUser;
      const sid = await createSahne(u?.uid ?? "", ad.trim() || `${vw.name} sahnesi`, u?.displayName || u?.email || undefined);
      const link = `${window.location.origin}/sahne/${sid}`;
  ...
  // "Görsel / Video" AYRI DÜĞME DEĞİL (kullanıcı kararı, kütüphane yeniden
  // kurulumunun 1. adımı): veri merkezi KÜTÜPHANE — bir şey almak isteyen oraya
  // girer, yüklemek isteyen ORADAN yükler. İki kapı iki ayrı zihin haritası
  // yaratıyordu ("yüklediğim nereye gitti?").
        ...
        <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={() => setLibOpen(false)}>
          <div className="bg-white border border-line rounded-2xl p-5 w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            {/* İKİ KAYNAK: "Bu ekran" (medya[] kaydı) ve "Ortak raf" (tüm
                ekranların havuzu — flowsign/ortak, ekran klasörlerinden YAPISAL
            ...
            {/* YÜKLEME KAPISI BURADA (kullanıcı kararı, kütüphane kurulumunun
                ...
                gibi hem yükler hem alana koyar. Raf sekmesindeyken hedef RAFTIR. */}
            {queue ? (
                  ...
                  {rafGoster.map((d) => (
                    ...
                    >
                      ...
                      >
                        <span className="block aspect-square relative">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={d.kind === "video" ? stillOf(d.src) || undefined : cldFit(d.src, 200)} alt="" className="w-full h-full object-cover bg-black" />
                          {d.kind === "video" && <span className="absolute bottom-1 right-1 text-xs">🎬</span>}
