...
function BosAlan() {
  return (
    <div className="w-full h-full grid place-items-center select-none" aria-hidden>
      ...
      <img src="/logo-o-sign-white.png" alt="" className="max-w-[38%] max-h-[38%] w-auto h-auto opacity-[0.13]" />
    </div>
  );
}
...
function GomuluEkran({ hedef, box, zincir }: { hedef: { id?: string; slug?: string }; box: { w: number; h: number }; zincir: string[] }) {
  ...
  useEffect(() => {
    ...
    if (hedef.id) return watchVideowall(hedef.id, setVw);
    if (hedef.slug) return watchVideowallBySlug(hedef.slug, setVw);
...
function ZonePlayer({
  ...
  return (
    ...
    >
      ...
      {/* Önden indirme DİSİPLİNİ: her alan sıradaki videoyu tam indirirse üç
          videolu bir ekranda üç fazladan indirme + üç fazladan çözücü olur.
          Küçük alanda geçiş zaten göze batmaz — orada yalnız üstbilgi alınır. */}
      {next?.kind === "video" && next.src && (
        <video key={next.src} src={next.src} preload={designPx.w >= 900 ? "auto" : "metadata"} muted playsInline className="hidden" aria-hidden />
      )}
    </div>
  );
}
