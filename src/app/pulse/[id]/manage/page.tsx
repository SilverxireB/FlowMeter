...
export default function PulseManagePage() {
  ...
  useEffect(() => {
    refreshDays();
  }, [refreshDays, today?.total]); // yeni oy → bugün değişir → trendi tazele
  useEffect(() => {
  ...
  async function pdfReport() {
    ...
    docPdf.text(`FlowPulse Raporu — ${pulse.title}`, 14, 18);
    docPdf.setFontSize(10);
    docPdf.text(`Soru: ${pulse.question.text}`, 14, 26);
      ...
      <section className="max-w-5xl mx-auto px-4 py-8 flex flex-col gap-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="card p-5 text-center">
            ...
            {delta !== null ? (
              <p className={`font-display font-bold text-5xl tabular-nums ${delta >= 0 ? "text-emerald-600" : "text-brand"}`}>
          ...
          <div className="card p-5">
            ...
            <div className="flex items-end gap-1 h-36">
              {days.map((d) => {
                ...
                return (
                  <div key={d.id} className="flex-1 min-w-0 group relative">
                    <div className="rounded-t" style={{ height: `${Math.max(4, p) * 1.4}px`, background: scoreColor(p) }} />
                    <span className="absolute -top-7 left-1/2 -translate-x-1/2 hidden group-hover:block bg-ink text-white text-[10px] rounded px-1.5 py-0.5 whitespace-nowrap tabular-nums">
                      {d.id.slice(5)} · %{p} · {d.total} oy
                    </span>
                  </div>
                );
              })}
        ...
        {heat && (
          <div className="card p-5 overflow-x-auto">
            <p className="eyebrow mb-1">Gün × saat ısısı <span className="normal-case font-normal text-muted">(son 30 gün — hangi vardiyada ne oluyor?)</span></p>
            <div className="mt-3 grid" style={{ gridTemplateColumns: "38px repeat(24, minmax(14px, 1fr))", gap: 2 }}>
              <span />
              ...
              {[1, 2, 3, 4, 5, 6, 0].map((wd) => (
                <>
                  <span key={`n${wd}`} className="text-[10px] text-muted font-semibold self-center">{DAY_NAMES[wd]}</span>
                  {heat[wd].map((cell, h) => {
                    const { min, max } = type === "smiley" ? { min: 1, max: 5 } : type === "nps" ? { min: 0, max: 10 } : { min: 0, max: 1 };
                    const p = cell.t ? Math.round((((cell.s / cell.t) - min) / (max - min)) * 100) : null;
                    return (
                      <span
                        key={`${wd}-${h}`}
                        title={p === null ? "veri yok" : `${DAY_NAMES[wd]} ${h}:00 · %${p} (${cell.t} oy)`}
                        className="h-4 rounded-[3px]"
                        style={{ background: p === null ? "#f1f0ef" : scoreColor(p), opacity: p === null ? 1 : 0.85 }}
                      />
                    );
                  })}
                </>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="card p-5 flex flex-col gap-3">
            ...
            <label className="text-sm flex flex-col gap-1">
              <span className="text-muted text-xs">Soru metni</span>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <label className="text-sm flex flex-col gap-1">
                <span className="text-muted text-xs">Kiosk freni (sn)</span>
          ...
          <div className="card p-5 flex items-start gap-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-muted text-xs leading-relaxed mb-2">
                Postere bas, insanlar telefonla okutup oy versin. Kiosk linkini tablette aç, köşeye <b>5 dokunuş</b> + PIN ile çıkılır.
