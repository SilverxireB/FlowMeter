...
import { gunKey, izleGunSiparisleri } from "@/lib/kantin/api";
...
export default function KantinPanoPage() {
  ...
  useEffect(() => {
    if (!seciliId) return;
    return izleGunSiparisleri(seciliId, gunKey(), (s) => {
  ...
  const hazirlanan = useMemo(() => liste.filter((s) => s.durum === "hazirlaniyor" || s.durum === "yeni"), [liste]);
